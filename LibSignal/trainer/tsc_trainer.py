...
class TSCTrainer(BaseTrainer):
    ...
    def create_metrics(self):
        ...
        if Registry.mapping['command_mapping']['setting'].param['delay_type'] == 'apx':
            ...
            world_metrics = ['real avg travel time', 'throughput','average wating time']
        else:
            lane_metrics = ['rewards', 'queue']
            world_metrics = ['delay', 'real avg travel time', 'throughput','average wating time']
    ...
    def train(self):
        ...
        for e in range(self.episodes):
            ...
            _reward.append(self.metric.rewards())
            aql.append(self.metric.queue())
            awt.append(self.metric.average_waiting_time())
            thr.append(self.metric.throughput())
        ...
        np.savetxt("reward.txt", _reward)
        np.savetxt("queue.txt", aql)
        np.savetxt("awt.txt", awt)
        np.savetxt("throughput.txt", thr)
        # self.dataset.flush([ag.replay_buffer for ag in self.agents])
